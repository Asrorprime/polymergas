...
class CurrentOrderLinksZeroComponent extends React.Component {
  state: initialState;
  props: any;
  ...
  render() {
    const {orders, changeOrder, setMyState, setShowHistoryFunctionState,changeOrderSum,openChangeModal, pushNextModal, showHistoryModal, changeToPayed} = this.props
        ...
        <div className="mt-4">
          <Row>
            {orders ? orders.map((item: any, index: number) =>
              <Col md={6} key={index} className="my-3">
                          ...
                          <div className="h-100">
                            <div className="d-flex align-items-center h-100 mb-2 mt-1 userInfos">
                              <div className="">
                                ...
                                <span className="icon icon-user mr-2"
                                  ...
                                  <button
                                    ...
                                    onClick={
                                      () => {
                                        pushNextModal();
                                        setMyState("myFunction",
                                          () => changeOrder(item.userOrderId, 1))
                                      }}>
                                    <span className="icon icon-arrow-right-line"/>
                                  </button>
                                  <UncontrolledTooltip placement="right" target="toInProgress">
                                    Buyurtmani yetkazish bosqichiga o`tkazish
                                  </UncontrolledTooltip>
                                </div>
                                {item.payed ? <div className="change-order-btn btn-size-md ml-3">
                                    <button onClick={() => {
                                      pushNextModal();
                                      setMyState("myFunction", () => changeToPayed(item.userOrderId, false, 0))
                                    }}>
                                  ...
                                  <div className="change-order-btn btn-size-md ml-3">
                                    <button onClick={() => {
                                      pushNextModal();
                                      setMyState("myFunction", () => changeToPayed(item.userOrderId, true, 0))
                                    }}>
                              ...
                              <div className="d-flex align-items-center my-2">
                                <div className="change-order-btn btn-size-md">
                                  <button
                                      ...
                                      onClick={() => {
                                        openChangeModal();
                                        setMyState("myFunction", () => changeOrderSum(item.userOrderId))
                                      }}>
                                    <span className="icon icon-check"/>
                                  </button>
                                  <UncontrolledTooltip placement="right" target="change">
...
export default CurrentOrderLinksZeroComponent;
